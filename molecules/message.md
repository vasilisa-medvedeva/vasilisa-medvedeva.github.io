...
> A full chat message bubble: the Bubble shape filled with an optional sender name, an optional "forwarded from" header, the message text, and the BubbleBottom strip — plus an avatar for channel messages.
...
└── .bubble2 (+--white --left, +--top/--mid/--last)   Bubble shape
    └── .bubble2__content
...
| position | top · middle · bottom · alone → Bubble grouping (tail on bottom/alone) |
...
| Token | Role |
|---|---|
| `--color-brand-yellow-16` / `-32` | Sender bubble fill / stroke (via Bubble) |
| `--color-constant-primary` / `--color-brand-yellow-24` | Recipient fill / stroke (via Bubble) |
...
| Component | Role |
|---|---|
| Bubble | bubble shape, grouping positions, white/left |
| BubbleBottom | time / reactions strip |
| InfoTitle (forward) | "Forwarded from" header |
| AvatarPlaceholder (S) | channel avatar |

## Do / Don't
✅ **Do** — let Bubble draw the shape from content; just fill `.bubble2__content`.
✅ **Do** — show the name only on the first grouped bubble, the avatar only on the last.
❌ **Don't** — give the bubble a CSS background/border — Bubble's SVG path is the skin.
